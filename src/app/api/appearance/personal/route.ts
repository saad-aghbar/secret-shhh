...
import { clearPersonalAppearance, savePersonalAppearance } from "@/lib/appearance/service";
...
export async function PUT(request: Request) {
  ...
  try {
    const appearance = await savePersonalAppearance({
      requestId,
    ...
    logError({
      requestId,
      operation: "PUT /api/appearance/personal",
      userId: session.user.id,
...
export async function DELETE() {
  ...
  try {
    const appearance = await clearPersonalAppearance({
      requestId,
      userId: session.user.id,
      conversationId: session.conversationId,
    });
    ...
    logError({
      requestId,
      operation: "DELETE /api/appearance/personal",
      userId: session.user.id,
      errorClass: error instanceof Error ? error.name : "unknown",
    });
    return internalError();
