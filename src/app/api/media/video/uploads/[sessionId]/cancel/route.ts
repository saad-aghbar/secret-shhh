...
import { cancelVideoUpload } from "@/lib/media/video-upload-service";
...
export async function POST(_request: Request, { params }: Params) {
  ...
  try {
    const result = await cancelVideoUpload({
      requestId,
    ...
    logError({
      requestId,
      operation: "POST /api/media/video/uploads/[sessionId]/cancel",
      userId: session.user.id,
